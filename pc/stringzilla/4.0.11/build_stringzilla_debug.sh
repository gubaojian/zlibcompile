...
##  -DBUILD_TESTING=OFF
 mkdir build
 cd build
 cmake  -DCMAKE_BUILD_TYPE=Debug -DSTRINGZILLA_INSTALL=ON -DSTRINGZILLA_BUILD_BENCHMARK=OFF -DSTRINGZILLAS_BUILD_SHARED=OFF -DSTRINGZILLA_BUILD_TEST=OFF -DSTRINGZILLA_BUILD_CUDA=OFF -DCMAKE_INSTALL_PREFIX=$INSTALL_DIR  ..
 make -j2
 make install
 cd ..
